...
- **`src/gui.py`**:
  ...
  - **Кнопки действий в кластере**:
    - **Оставить первый, удалить остальные**: Помечает все изображения, кроме первого, как `to_delete=True`.
    - **Игнорировать**: Помечает кластер как просмотренный (`reviewed=True`), не помечая файлы к удалению.
    - **Удалить все**: Помечает все изображения в кластере как `to_delete=True`.
  - **Контекстное меню изображения (правый клик)**:
    - **Пометить на удаление**: Помечает конкретное изображение как `to_delete=True`.
    - **Снять пометку на удаление**: Снимает с изображения пометку `to_delete=True`.
    - **Оставить это, остальные удалить**: Оставляет только выбранное изображение, а все остальные в кластере помечает к удалению.
    - **Показать в оригинальном размере**: Открывает изображение в новом окне для просмотра в полном размере.
...
- **Функционал:**
  ...
  - **Меню "Команды"**:
    - **Отменить все изменения**: Снимает пометки `to_delete` и `reviewed` со всех записей в базе данных.
    - **Удалить все помеченные файлы**: Переименовывает все файлы, помеченные как `to_delete=True`, добавляя суффикс `._deleted`. Требует подтверждения.
